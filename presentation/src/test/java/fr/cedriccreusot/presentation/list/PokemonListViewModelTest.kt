package fr.cedriccreusot.presentation.list
...
class PokemonListViewModelTest {
    ...
    fun `when we observe the pokemonList it should start loading then return the list of pokemons`() {
        ...
        val observe = mockk<Observer<fr.cedriccreusot.presentation.list.State>>(relaxed = true)

        val viewModel = fr.cedriccreusot.presentation.list.PokemonListViewModel(useCase)

        viewModel.pokemonList().observeForever(observe)
        viewModel.fetchPokemons()

        verify { useCase.invoke() }
        verify(atMost = 2) { observe.onChanged(fr.cedriccreusot.presentation.list.State.Loading) }
        verify { observe.onChanged(fr.cedriccreusot.presentation.list.State.Success(pokemonList)) }
    ...
    fun `when we observe the pokemonList and the usecase return an error it should return an error state`() {
        ...
        verify { useCase.invoke() }
        verify(atMost = 2) { observe.onChanged(fr.cedriccreusot.presentation.list.State.Loading) }
        verify { observe.onChanged(fr.cedriccreusot.presentation.list.State.Error("Empty Pokemon list")) }
        confirmVerified(useCase, observe)
    }
}